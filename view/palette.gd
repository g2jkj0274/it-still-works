...
##
## 0.045 였다. 그 폭으로는 **지표가 통째로 한 덩어리 초록**이었다. 스크린샷
## 열일곱 장 가운데 넷이 화면의 칠 할을 같은 초록으로 덮고 있었고, 스토어
## 목록 크기에서 그것은 미완성으로 읽힌다. 두 배로 넓혀도 종류는 여전히
## 알아볼 수 있다 — 가르는 것은 색상이지 명도가 아니기 때문이다.
...
## 켜진 등. 스스로 빛나면서 빛도 뿌리므로 몸통 색까지 밝으면 흰 상자가 된다.
## 15_underground 에서 등 셋이 전부 갓 없는 흰 덩어리로 날아가 있었다.
const LAMP_LIT := Color(0.96, 0.90, 0.66)
...
## 등과 관솔불이 뿌리는 빛의 빛깔.
##
## 몸통 색을 그대로 쓰고 있었다. 그런데 빛은 바닥 색에 곱해지므로, 노란빛이
## 풀밭에 떨어지면 **형광 초록 웅덩이**가 된다. 밤 그림에서 그것은 불이
## 아니라 이물질로 보였다. 주황 쪽으로 옮겨 풀의 초록과 겨루게 한다.
##
...
## 위협.
##
## 연보라였다. 밤은 화면 전체가 청록으로 덮이는데 그 위에서 연보라가 배경에
## 붙어 **밤 그림에서 위협이 보이지 않았다.** 밤의 긴장이 이 게임의 유일한
## 압박인데 화면에 없었던 것이다. 밤 색과 반대쪽인 분홍으로 옮긴다.
const THREAT := Color(0.90, 0.62, 0.78)
